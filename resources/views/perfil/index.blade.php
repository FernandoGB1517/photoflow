...
@section('titulo')
    <span class="text-gray-500">Editar Perfil: </span> <em>{{ auth()->user()->username}}</em>
@endsection
        ...
        <div class="md:w-1/2 bg-white shadow p-6">
            <form method="POST" action="{{ route('perfil.store') }}" enctype="multipart/form-data" class="mt-10 md:mt-0">
                    ...
                    <select id="comunidad_autonoma" name="comunidad_autonoma" class="border p-3 w-full rounded-lg bg-gray-100">
                        ...
                        @foreach($comunidades as $comunidad)
                            <option value="{{ $comunidad->idCCAA }}" 
                                {{ $comunidad->idCCAA == $usuario->idCCAA ? 'selected' : '' }}>
                                {{ $comunidad->Nombre }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="mb-5">
                    ...
                    <select id="provincia" name="provincia" class="border p-3 w-full rounded-lg bg-gray-100">
                        @if($usuario->idProvincia)
                            <option value="{{ $usuario->idProvincia}}" selected>{{ $provinciaUsuario->Provincia }}</option>
                        @else
                ...
                <div class="mb-5">
                    ...
                    <select id="localidad" name="localidad" class="border p-3 w-full rounded-lg bg-gray-100">
                        @if($usuario->idMunicipio)
                            <option value="{{ $usuario->idMunicipio}}" selected>{{ $localidadUsuario->Municipio }}</option>
                        @else
                ...
                <div class="text-center mt-5">
                    <a href="{{ url()->previous() }}"  class="bg-sky-600 hover:bg-sky-700 transition-colors cursor-pointer uppercase font-bold w-full rounded-lg p-3 text-white">Cancelar</a>
                </div>
            </form>
        </div>
